...
from expands.tools import  nested_method,import_object
...
class CeleryClient(Singleton):
	...
	def enable_events(self):
		return self.application.control.enable_events()
	
	@property	
	def disable_events(self):
		return self.application.control.disable_events()
	...
	def workers(self):
		...
		if isinstance(statuses,dict):
			for name, info in response.items():
				...
				worker['complete'] = sum([v for v in info['total'].values()])
	...
	def query_task(self,ids):
		response = self._inspect.query_task(*ids)
		if not  response:
			return {}
		else:
			return response
